...
- Native decentralized protocol engine bundling now has a manifest-backed readiness contract and local packaging/smoke scripts; user-installed protocol prerequisites are not acceptable as the default path.
...
- `pnpm --filter @browser/storage-adapters build`, `pnpm --filter @browser/storage-adapters lint`, and `pnpm --filter @browser/storage-adapters test` passed on `main` with 16 Node tests.
- `./scripts/dweb-engines/smoke.sh` passed on `main`.
- `npm test && npm run build` passed in `./web`; Vite reported existing Browserslist age and large chunk warnings.
...
- [x] Reconcile #146 against current code, close stale review-hardening state, and make every trust-gap marker either implemented or explicitly scoped as production hardening outside the open issue.
- [x] Close #133 by making native protocol engine readiness machine-checkable rather than plan-only.
- [x] Re-scan active Swift/runtime/docs temporary markers and remove vague TODO/FIXME language from production paths.
- [x] Add focused tests for new functionality.
- [x] Verify the focused Swift test lane, full `dBrowserTests`, macOS build, DWeb smoke test, and touched Rust crates locally.
- [x] Commit, push, and close all resolved GitHub issues.

Validation notes:

- Focused Swift tests for Solana vote-root verification and DWeb manifest readiness passed.
- `xcodebuild test -project swift/dBrowser/dBrowser.xcodeproj -scheme dBrowser -destination 'platform=macOS,arch=arm64' -derivedDataPath /tmp/dBrowser-open-issues-tests -only-testing:dBrowserTests` passed.
- `xcodebuild build -project swift/dBrowser/dBrowser.xcodeproj -scheme dBrowser -destination 'platform=macOS,arch=arm64' -derivedDataPath /tmp/dBrowser-open-issues-build` passed.
- `node --test services/storage-adapters/src/handlers.test.mjs` and `./scripts/dweb-engines/smoke.sh` passed.
- `cargo check -p p2p -p afm-node` passed.
...
- [x] 2b. Swift 6 language mode adopted by the user (app target `SWIFT_VERSION = 6.0`). Resolved the resulting hard errors: marked the value-type `MCPServerConfiguration`/`MCPServerStatus`/`BlockchainCapabilityGrant` initializers, statics (`disabled`, `defaultForMCPServer`, `defaultChainRefs`, `defaultApprovalGates`), and `normalizedChainRefs` `nonisolated`, and made the `dBrowserTests` suite `@MainActor`. Full app build and `dBrowserTests` pass under Swift 6.
- [x] 3. Began decomposing the `ContentView` god file: extracted the Local LLM panel cluster (`LocalLLMPanelViews.swift`, 8 views) and the wallet/identity/payments/AFM cluster (`WalletPanelViews.swift`, 13 views) into per-domain files, taking `ContentView.swift` from 4067 to 2769 lines and promoting the shared `platformBackgroundColor`/`PanelHeaderView`/`EmptyPanelView`/`FlowPillRow` helpers to internal. Additional panel clusters and per-domain stores are architecture hygiene outside #146, not review blockers.
- [x] 4. Replaced the hard-coded `/Users/johansellstrom/.../Broom/...gemma-4-e2b-it-4bit-mlx` MLX path with a relative `localWorkspaceRelativePath` plus a `localWorkspaceCandidateURLs` resolver: `DBROWSER_MLX_MODEL_DIR` (explicit dir) and `DBROWSER_MLX_WORKSPACE`/`BROOM_WORKSPACE` (workspace roots) env overrides, then the home-relative `~/dev/advatar/Broom` developer checkout, then `~/Library/Application Support/dBrowser/models/<dir>`. Made the two model-presence tests CI-safe (accept the Hugging Face fallback when the artifact is absent) and added a deterministic env-override resolution test.
...
- [x] B. Relabel the `*LightClient` types to reflect RPC/gateway trust delegation (per user direction: accurate per-type documentation rather than a blanket symbol rename, since a blanket rename would mislabel clients that do real work, e.g. Bitcoin header PoW). Added a "Trust boundary (goal: minimize remote trust)" doc to each `*LightClientServiceClient` stating its real status: Bitcoin (real header PoW/hash-linkage, not yet end-to-end SPV), Cosmos (real Tendermint header verification where a bundle is supplied), Substrate/Tron/Avalanche/XRPL/Move (local quorum verification over signed service evidence), EVM (finality/proofs modeled, RPC fallback by default), and Solana (local vote-root proof bundles verify Ed25519 stake quorum when supplied). All default to `.rpcFallback`/`.gatewayRPCFallback`.

## Trust Gap Closure (real light-client verification)

- [x] Bitcoin SPV proof-of-work verification (first chain off blind RPC trust). Added `BitcoinProofOfWork` (real compact-`nBits` -> 256-bit target decode and big-endian hash comparison) and `BitcoinBlockHeader.meetsProofOfWork`, enforced proof-of-work in `BitcoinTransactionInclusionProof.verify()` so a transaction inclusion is accepted only against a header that did real work (a forged/underworked header with any Merkle root is now rejected), and added `BitcoinProofOfWork.validatesHeaderChain` for header-chain PoW + linkage validation. Tested with the real mainnet genesis block (meets its target), a harder-target negative, an underworked self-consistent header (rejected), and chain linkage. Production hardening outside #146: anchor the proven header chain to tip from a trusted checkpoint, validate difficulty retargeting across epochs, and wire local-proof results into chain-trust elevation in place of the remote-reported sync state.
- [x] Cosmos Tendermint commit-signature verification (Ed25519). The two-thirds voting-power threshold is now computed from validators whose commit signature cryptographically verifies (`TendermintCommitSignatureVerifier`, Curve25519/CryptoKit) over a canonical vote encoding — a `signed` flag without verifiable bytes no longer counts, and a signature by the wrong key is rejected. Existing fixtures re-signed with real deterministic keys; added a gap-closure test (flag-only + forged signatures rejected). Production hardening outside #146: drive bundle verification on the default path and align the canonical vote bytes with Tendermint's CanonicalVote protobuf.
- [x] BLS12-381 brought in: vendored the audited `supranational/blst` as a local SwiftPM package `swift/BLSVerifierKit` (submodule + pure-C amalgamated build) exposing `BLSVerifier.fastAggregateVerify` (minimal-pubkey-size, Ethereum DST). Validated standalone with `swift test`: real keygen→sign→verify, tamper/wrong-key rejection, and two-signer aggregate verification.
- [x] EVM sync-committee verification: `BLSVerifierKit` is wired into the app target and `EVMSyncCommitteeVerifier` verifies the committee's aggregate signature with real BLS12-381 (`BLSVerifier.fastAggregateVerify`), requiring a two-thirds participation supermajority; a verified update reports `.proofChecked` (local proof), insufficient/invalid/malformed updates do not. Added `BLSSigner` (keygen/sign/aggregate) for test-vector generation and four app tests (real-signature verify, insufficient participation, wrong-root rejection, malformed). Precision hardening outside #146: snapshot providers pass an exact SSZ `hash_tree_root` + `compute_domain` signing root (genesis validators root, fork version), and live EVM snapshot flow should supply that bundle.
...
- [x] Solana: local vote-root proof bundles now bind slot/blockhash/account-root/transaction-status-root to a validator set hash and require verified Ed25519 validator signatures over the vote root to exceed two-thirds effective stake. Flag-only and wrong-key votes are rejected.

North star (user direction 2026-06-08): avoid trusting remote services to the extent possible. No advertised chain currently has a production light client that proves state to tip end-to-end; Bitcoin, Cosmos, EVM, Substrate, Tron, Avalanche, XRPL, Move, and Solana now have partial real verification logic, and all default to RPC/gateway fallback unless local proof bundles are supplied. Replacing the RPC fallback with real local verification (SPV, sync-committee/account/storage proofs, Tendermint/GRANDPA to the default path) is the tracked direction.
- [x] C. Keychain wallet seed storage: added `KeychainWalletSeedStore` (system Keychain, `AfterFirstUnlockThisDeviceOnly`, in-memory fallback when unentitled) and `WalletSeedFactory.generateSeedHex` (256-bit secure entropy). `createEmbeddedWallet` now loads-or-creates the seed from secure storage instead of holding a `UUID().uuidString` in memory. Added a hermetic round-trip test. Secure Enclave key-wrapping of the seed and BIP-39 mnemonic encoding are intentionally absent until the wallet approval UX defines recovery and hardware-binding policy.
- [x] D. `BrowserViewModel` decomposition (incremental, per user direction). Extracted the smart-history domain into `BrowserHistoryService` (recording, summarization + exclusion config, autocomplete ranking, recall, persistence), which now owns the `excludedDomains` state and `SmartHistoryStore`. The view model keeps the observable `history` array and delegates, dropping from 1580 to 1422 lines with all 189 tests green. Additional LLM/Copilot orchestration, wallet, and OpenMind memory splits are architecture hygiene outside #146, not review blockers.
...
- [x] Commit and push the scoped changes, then close #189 with validation evidence.

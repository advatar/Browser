...
- [~] 3. Began decomposing the `ContentView` god file: extracted the Local LLM panel cluster (`LocalLLMPanelViews.swift`, 8 views) and the wallet/identity/payments/AFM cluster (`WalletPanelViews.swift`, 13 views) into per-domain files, taking `ContentView.swift` from 4067 to 2769 lines and promoting the shared `platformBackgroundColor`/`PanelHeaderView`/`EmptyPanelView`/`FlowPillRow` helpers to internal. Follow-up: extract more panel clusters and split `BrowserViewModel`'s 34 `@Published` flat state into per-domain stores (`TabStore`/`LLMCoordinator`/`WalletBridge`/`MemoryCoordinator`).
...
- [ ] EVM sync-committee verification: wire `BLSVerifierKit` into the app target (Xcode local-package reference) and implement the beacon sync-committee path (participation bits, supermajority, signing root + domain) on top of `BLSVerifier`. Foundation is done; this is the remaining integration.
- [ ] Replace fixture quorum checks for Solana / Avalanche / Tron / XRPL / Sui / Aptos with real verification.

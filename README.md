...
Private-overlay browsing recognizes Tor onion services, I2P, Hyphanet/Freenet, ZeroNet, and Lokinet before search or HTTPS fallback and routes them only to local/private adapters. Runtime status now distinguishes configured, running, reachable, blocked, misconfigured, and verified adapters. Full dark-web protocol support still requires bundled or managed runtimes plus end-to-end smoke tests for each network; routing support alone should not be described as complete Tor/I2P/etc. runtime support.

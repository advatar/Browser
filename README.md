...
Private-overlay browsing recognizes Tor onion services, I2P, Hyphanet/Freenet, ZeroNet, and Lokinet before search or HTTPS fallback and routes them only to local/private adapters. Runtime status now distinguishes configured, managed-launch-ready, running, reachable, blocked, misconfigured, and verified adapters. The first managed runtime slice discovers and can launch a local Tor/Arti process with loopback-only SOCKS binding and no DNS or clearnet fallback policy, but adapter health must still verify before `.onion` navigation proceeds. Full dark-web protocol support still requires managed runtimes and end-to-end smoke tests for every network; routing support alone should not be described as complete Tor/I2P/etc. runtime support.

    ...
    B: BlockT,
{
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        let sync_handle = tokio::spawn(async move { Self::background_sync_loop(client).await });
                    ...
                    match extrinsics_result {
                        Ok(extrinsics) => {
                            ...
                            match collected {
                                ...
                                Err(e) => {
                                    log::warn!(
                                        "Failed to collect extrinsics for block #{}: {}",
                                        number,
                                        e
                                    );
                                    None
    ...
    async fn background_sync_loop(client: Arc<OnlineClient<PolkadotConfig>>) -> Result<()> {
        ...
        loop {
            ...
            if consecutive_errors > 5 {
                log::warn!(
                    "Too many consecutive errors ({}), pausing sync for recovery",
                    consecutive_errors
                );

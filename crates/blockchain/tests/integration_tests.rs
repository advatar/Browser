...
async fn test_client_connection() -> Result<()> {
    with_test_node_async(|config| async move {
        ...
        let genesis_hash = client.inner().rpc().genesis_hash().await?;

        assert!(!genesis_hash.is_zero(), "Genesis hash should not be zero");

        // Get the runtime version
        let runtime_version = client.inner().rpc().runtime_version(None).await?;

        assert!(
            !runtime_version.spec_name.is_empty(),
            "Runtime spec name should not be empty"
        );
...
fn test_wallet_operations() -> Result<()> {
    with_test_node(|_| {
        ...
        let key_pair = KeyPair::from_seed(&[0u8; 32], KeyType::Sr25519)?;
...
async fn test_chain_sync() -> Result<()> {
    with_test_node_async(|config| async move {
        ...
        let sync_handle = tokio::spawn(async move { chain_sync.start().await });
...
async fn test_transaction_submission() -> Result<()> {
    with_test_node_async(|config| async move {
        ...
        let initial_balance = client.inner().fetch_or_default(&bob, None).await?;
        ...
        let _block_hash = client.inner().rpc().author_wait_for(tx_hash).await?;

        // Wait for a few blocks to ensure the transaction is finalized
        wait_for_blocks(&client, 3).await?;

        // Check the new balance of Bob
        let new_balance = client.inner().fetch_or_default(&bob, None).await?;
...
async fn test_wallet_integration() -> Result<()> {
    with_test_node_async(|config| async move {
        ...
        let key_pair = KeyPair::from_seed(&[1u8; 32], KeyType::Sr25519)?;

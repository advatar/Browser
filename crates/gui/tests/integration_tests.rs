...
use gui::{
    ...
    security::{PrivacySettings, SecurityManager},
};
...
async fn test_protocol_handler_integration() -> Result<()> {
    ...
    // IPFS hashes validate locally even without network fetches
    assert!(handler.is_valid_ipfs_hash("QmYjtig7VJQ6XsnUjqqJvj7QaMcCAwtrgNdahSiFofrE7o"));

    // Invalid IPFS hashes should error
    assert!(handler.resolve_url("ipfs://invalid_hash").await.is_err());
...
async fn test_tauri_app_integration() -> Result<()> {
    // Requires the `test` feature on Tauri; skip when unavailable.
    #[cfg(feature = "test")]
    {
        ...
        let result = app
            .invoke("create_tab", json!({ "url": "https://test.com" }))
            .await;
        assert!(result.is_ok());
...
async fn test_concurrent_tab_operations() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        let handle =
            tokio::spawn(
                async move { engine_clone.create_tab(format!("https://example{}.com", i)) },
            );
        handles.push(handle);
    }
    ...
    for tab_id in tab_ids {
        let engine_clone = engine.clone();
        let handle = tokio::spawn(async move { engine_clone.close_tab(&tab_id) });
...
async fn test_data_persistence_simulation() -> Result<()> {
    ...
    engine1.add_bookmark(
        "Persistent".to_string(),
        "https://persistent.com".to_string(),
        None,
        vec![],
    )?;
    engine1.add_to_history(
        "https://persistent.com".to_string(),
        "Persistent".to_string(),
    )?;

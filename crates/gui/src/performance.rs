...
impl PerformanceMonitor {
    ...
    async fn collect_memory_metrics(&self) -> Result<MemoryMetrics> {
        ...
        let used_mem_kib = if sys.used_memory() > 0 { sys.used_memory() } else { sys.total_memory().saturating_sub(sys.available_memory()) };
    ...
    async fn collect_rendering_metrics(&self) -> Result<RenderingMetrics> {
        if let Ok(stats) = self.rendering_stats.lock() {
            let fps = if stats.frame_times.is_empty() {
                ...
                let avg_frame_time: Duration = stats.frame_times.iter().sum::<Duration>() / stats.frame_times.len() as u32;
...
{}
    ...
    fn generate_recommendations(&self, metrics: &PerformanceMetrics) -> String {
        ...
        let memory_usage_percent = (metrics.memory_usage.heap_used as f64 / metrics.memory_usage.heap_total as f64) * 100.0;
        if memory_usage_percent > 80.0 {
            recommendations.push("⚠️  High memory usage detected. Consider closing unused tabs or clearing cache.");
        }
        
        // CPU recommendations
        let cpu_usage = metrics.cpu_usage.user_time + metrics.cpu_usage.system_time;
        if cpu_usage > 50.0 {
            recommendations.push("⚠️  High CPU usage detected. Check for resource-intensive scripts or extensions.");
        }
        
        // FPS recommendations
        if metrics.rendering_metrics.frames_per_second < 30.0 {
            recommendations.push("⚠️  Low frame rate detected. Consider reducing visual effects or closing tabs.");
        }
        
        // Network recommendations
        if metrics.network_metrics.failed_requests > 10 {
            recommendations.push("⚠️  High number of failed network requests. Check internet connection.");
        }
    ...
    pub fn export_metrics_json(&self) -> Result<String> {
        let history = self.get_metrics_history()?;
        serde_json::to_string_pretty(&history).map_err(|e| anyhow::anyhow!("JSON serialization failed: {}", e))
    }

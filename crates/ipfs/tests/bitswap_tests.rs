...
async fn test_bitswap_block_exchange() -> Result<()> {
    ...
    assert!(
        !node2_addrs.is_empty(),
        "Node2 should have listening addresses"
    );
    ...
    assert_eq!(
        retrieved_data,
        Some(test_data),
        "Retrieved data should match the original data"
    );
    ...
    let handles: Vec<_> = blocks
        ...
        .map(|(cid, expected_data)| {
            ...
            tokio::spawn(async move {
                let data = node_clone
                    .get_block(&cid, Some(Duration::from_secs(10)))
                    .await?;
                assert_eq!(
                    data,
                    Some(expected_data),
...
async fn test_bitswap_peer_disconnect() -> Result<()> {
    ...
    let handle = tokio::spawn(async move {
        node1_clone
            .get_block(&cid, Some(Duration::from_secs(30)))
            .await
    });
    ...
    assert!(
        result.is_none(),
        "Block transfer should have failed due to disconnection"
    );
    ...
    assert_eq!(
        retrieved,
        Some(large_data),
        "Block should be retrievable after reconnection"
    );
...
async fn test_bitswap_wantlist_management() -> Result<()> {
    ...
    assert!(
        ...
        "Block should be in the blockstore"
    );
    ...
    let handle = tokio::spawn(async move {
        node_clone
            .get_block(&non_existent_cid, Some(Duration::from_secs(5)))
            .await
    });
    ...
    assert!(
        node.is_block_wanted(&non_existent_cid),
        "Block should be in wantlist"
    );
    ...
    assert!(
        !node.is_block_wanted(&non_existent_cid),
        "Block should be removed from wantlist"
    );
    ...
    assert!(
        node.wantlist_size() <= 5,
        "Wantlist should respect the size limit"
    );
...
async fn test_bitswap_duplicate_blocks() -> Result<()> {
    ...
    let handles: Vec<_> = (0..5)
        .map(|_| {
            ...
            tokio::spawn(async move {
                node_clone
                    .get_block(&cid, Some(Duration::from_secs(5)))
                    .await
            })
        })
        .collect();
    ...
    assert_eq!(
        ...
        "Block should only be transferred once"
    );
...
async fn test_bitswap_large_block() -> Result<()> {
    ...
    assert_eq!(
        retrieved,
        Some(large_data),
        "Retrieved large block should match original"
    );
...
async fn test_bitswap_block_provide() -> Result<()> {
    ...
    assert_eq!(
        retrieved,
        Some(test_data),
        "Should be able to retrieve provided block"
    );

...
async fn test_circuit_breaker_opens_on_consecutive_failures() -> Result<()> {
    ...
    for _ in 0..3 {
        ...
        assert!(
            result.is_ok(),
            "Should handle request even if block doesn't exist"
        );
...
async fn test_circuit_breaker_resets_after_timeout() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "Circuit breaker should be half-open and allow a request"
    );
    ...
    assert!(
        result.is_err(),
        "Circuit breaker should re-open after failed request"
    );

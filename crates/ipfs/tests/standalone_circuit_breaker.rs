...
impl CircuitBreaker {
    pub fn new(
        ...
        let state_metric = IntGauge::new(
            ...
            format!(
                "State of the {} circuit breaker (0=Closed, 1=HalfOpen, 2=Open)",
                name
            ),
        )
        .unwrap();
...
mod tests {
    ...
    fn test_metrics() {
        ...
        let state_metric = metric
            .iter()
            .find(|m| m.get_name() == "circuit_breaker_test_state")
            .unwrap();
        assert_eq!(
            state_metric.get_metric()[0].get_gauge().get_value(),
            CircuitState::Closed as f64
        );

...
use crate::ipfs::{
    Block, BlockStore, Config, Error, NodeEvent, Repo, Result,
};
use futures::{
            ...
            while let Some(command) = command_receiver.next().await {
                match command {
                    NodeCommand::Connect(peer_id, addrs) => {
                        ...
                        let _ = event_sender.clone().send(NodeEvent::PeerConnected(peer_id)).await;
                    }
                    NodeCommand::Disconnect(peer_id) => {
                        // TODO: Implement peer disconnection
                        let _ = event_sender.clone().send(NodeEvent::PeerDisconnected(peer_id)).await;
                    }
                    NodeCommand::AddBlock(block) => {
                        if let Err(e) = repo.put_block(block).await {
                            let _ = event_sender.clone().send(NodeEvent::Error(e.to_string())).await;
                        }
                    }

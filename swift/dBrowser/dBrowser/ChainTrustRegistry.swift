...
struct ChainTrustRegistry: Codable, Equatable {
    ...
    mutating func recordBitcoinLightClientSnapshot(_ snapshot: BitcoinLightClientServiceSnapshot) -> ChainTrustStatus {
    ...
    mutating func recordSolanaLightClientSnapshot(_ snapshot: SolanaLightClientServiceSnapshot) -> ChainTrustStatus {
        var status = snapshot.chainTrustStatus
        let existing = self.status(forChainRef: snapshot.cluster.chainRef)
        if status.evidence.isEmpty, let existingEvidence = existing?.evidence {
            status.evidence = existingEvidence
        }
        if status.latestCheckpoint == nil {
        ...
        if let index = statuses.firstIndex(where: { $0.matches(chainRef: snapshot.cluster.chainRef) }) {
            statuses[index] = status
        } else {
            statuses.append(status)
        }
    ...
    mutating func recordAvalancheLightClientSnapshot(_ snapshot: AvalancheLightClientServiceSnapshot) -> ChainTrustStatus {
        var status = snapshot.chainTrustStatus
        let existing = self.status(forChainRef: snapshot.network.chainRef)
        if status.evidence.isEmpty, let existingEvidence = existing?.evidence {
            status.evidence = existingEvidence
    ...
    mutating func recordTronLightClientSnapshot(_ snapshot: TronLightClientServiceSnapshot) -> ChainTrustStatus {
        var status = snapshot.chainTrustStatus
        let existing = self.status(forChainRef: snapshot.network.chainRef)
        if status.evidence.isEmpty, let existingEvidence = existing?.evidence {
            status.evidence = existingEvidence
    ...
    mutating func recordMoveLightClientSnapshot(_ snapshot: MoveLightClientServiceSnapshot) -> ChainTrustStatus {
        var status = snapshot.chainTrustStatus
        let existing = self.status(forChainRef: snapshot.chain.chainRef)
        if status.evidence.isEmpty, let existingEvidence = existing?.evidence {
            status.evidence = existingEvidence

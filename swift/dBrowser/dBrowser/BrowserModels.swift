...
enum BrowserAdvantageCategory: String, CaseIterable, Identifiable, Equatable {
    ...
    var title: String {
        ...
        case .developerAutomation: "Developer automation"
...
struct BrowserAdvantageScorecard: Equatable {
    ...
    static let current = BrowserAdvantageScorecard(
        capabilities: [
            ...
            BrowserAdvantageCapability(
                id: "developer-browser-evidence",
                category: .developerAutomation,
                title: "Local developer browser evidence",
                strawberryBaseline: "Aside's developer workflow article describes CI triage, PR evidence packets, staging screenshots, monitoring watches, and browser-console investigations.",
                dBrowserPosition: "dBrowser now models those workflows as local-first evidence runs with typed artifacts, redaction labels, source links, protected-action decisions, and Copilot-ready templates.",
                status: .exceeds,
                evidence: ["BrowserDeveloperWorkflowTemplate", "BrowserDeveloperEvidenceItem", "BrowserDeveloperWorkflowRun", "BrowserDeveloperApprovalPolicy"],
                action: BrowserAdvantageAction(
                    id: "developer-workflows",
                    title: "Run developer workflow",
                    detail: "Capture local evidence first and keep comments, flags, admin consoles, incident paging, deploys, billing, database, credentials, and wallet actions approval-gated.",
                    targetPanel: .copilot
                )
            ),
            BrowserAdvantageCapability(
...
enum BrowserDeveloperWorkflowKind: String, CaseIterable, Codable, Equatable, Identifiable {
    case failedCITriage
    ...
    var defaultEvidenceKinds: [BrowserDeveloperEvidenceKind] {
        switch self {
        case .failedCITriage:
    ...
    var protectedActions: [BrowserDeveloperProtectedAction] {
        switch self {
        case .failedCITriage:
...
enum BrowserDeveloperEvidenceKind: String, CaseIterable, Codable, Equatable {
    case screenshot
    case pageSnapshot
    case domSnapshot
    case logLink
...
enum BrowserDeveloperEvidenceRedactionState: String, Codable, Equatable {
    case none
    case redacted
    ...
    var title: String {
        ...
        case .sensitiveOmitted: "Sensitive content omitted"
        case .traceMinimized: "Trace-minimized"
        }
    }
}

enum BrowserDeveloperEvidencePrivacyBoundary: String, Codable, Equatable {
    case localOnly
...
struct BrowserDeveloperEvidenceItem: Codable, Equatable, Identifiable {
    let id: UUID
    var kind: BrowserDeveloperEvidenceKind
    var title: String
    var capturedAt: Date
    var summary: String
    var sourceURLString: String?
    ...
    var redactionState: BrowserDeveloperEvidenceRedactionState
    ...
    var boundarySummary: String {
        "\(privacyBoundary.title), \(redactionState.title)"
    }
}

enum BrowserDeveloperProtectedAction: String, CaseIterable, Codable, Equatable, Hashable {
    case postPRComment
...
enum BrowserDeveloperActionDisposition: String, Codable, Equatable {
    case localEvidenceOnly
    case draftOnly
    case requiresApproval
}

struct BrowserDeveloperApprovalDecision: Codable, Equatable, Identifiable {
    let id: String
    var action: BrowserDeveloperProtectedAction
    var disposition: BrowserDeveloperActionDisposition
    var reason: String

    init(action: BrowserDeveloperProtectedAction) {
...
enum BrowserDeveloperApprovalPolicy {
    static func decisions(for actions: [BrowserDeveloperProtectedAction]) -> [BrowserDeveloperApprovalDecision] {
        let uniqueActions = Array(Set(actions)).sorted { $0.rawValue < $1.rawValue }
        return uniqueActions.map(BrowserDeveloperApprovalDecision.init(action:))
    }

    static func requiresApproval(for actions: [BrowserDeveloperProtectedAction]) -> Bool {
        !decisions(for: actions).isEmpty
    }
}

enum BrowserDeveloperWorkflowStatus: String, Codable, Equatable {
    case draft
    case running
...
enum BrowserDeveloperWorkflowEntryPoint: String, Codable, Equatable, CaseIterable {
    case copilot
    case mcp
    case localREPL
    case routine
...
enum BrowserDeveloperAutomationSurfaceStatus: String, Codable, Equatable {
    case ready
    case staged
}

struct BrowserDeveloperAutomationSurface: Codable, Equatable, Identifiable {
    let id: BrowserDeveloperWorkflowEntryPoint
    var status: BrowserDeveloperAutomationSurfaceStatus
...
struct BrowserDeveloperWorkflowTemplate: Codable, Equatable, Identifiable {
    let id: String
    var kind: BrowserDeveloperWorkflowKind
    var title: String
    var prompt: String
    var localFirstInstructions: [String]
    var defaultEvidenceKinds: [BrowserDeveloperEvidenceKind]
    var protectedActions: [BrowserDeveloperProtectedAction]
    var entryPoints: [BrowserDeveloperWorkflowEntryPoint]
    ...
    func renderedPrompt(activeURLString: String?, snapshot: PageSnapshot?) -> String {
        var lines = [
            title,
            "",
            prompt,
        ...
        if let snapshot {
            lines.append("Snapshot already available: \(snapshot.title), \(snapshot.visibleText.count) text characters, \(snapshot.redactionCount) redactions.")
        }
    ...
    static let defaultLocalFirstInstructions = [
    ...
    static let localFirstDefaults: [BrowserDeveloperWorkflowTemplate] = [
...
struct BrowserDeveloperWorkflowRun: Codable, Equatable, Identifiable {
    let id: UUID
    var kind: BrowserDeveloperWorkflowKind
    var title: String
    var createdAt: Date
    var updatedAt: Date
    var status: BrowserDeveloperWorkflowStatus
    var entryPoint: BrowserDeveloperWorkflowEntryPoint
    var prompt: String
    var sourceURLString: String?
    var evidenceItems: [BrowserDeveloperEvidenceItem]
    var protectedActions: [BrowserDeveloperProtectedAction]
    var approvalDecisions: [BrowserDeveloperApprovalDecision]
    var localOutput: String?
    var copilotRunID: UUID?
    ...
    var requiresApprovalBeforeMutation: Bool {
    ...
    mutating func appendEvidence(_ item: BrowserDeveloperEvidenceItem, now: Date = Date()) {
        evidenceItems.append(item)
        updatedAt = now
    }

    mutating func markCompleted(summary: String, now: Date = Date()) {
        status = .completed
        localOutput = summary
        updatedAt = now
    }
    ...
    static func draft(
        ...
        return BrowserDeveloperWorkflowRun(
            ...
            prompt: template.renderedPrompt(activeURLString: activeURLString, snapshot: snapshot),
            sourceURLString: activeURLString,
        ...
        case .privateOverlayProtocols:
            RuntimeFeatureExplanation(
                ...
                detailPoints: [
                    ...
                    "Verified private-overlay status requires deterministic smoke fixture evidence with matching digest and no DNS, search, public gateway, or clearnet fallback assertions.",

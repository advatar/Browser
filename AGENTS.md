...
- Record user validation steps in `VALIDATE.md` so they can be handled async.
...
**Rule:**  
All system callbacks must be treated as **non-isolated** entry points.
...
## Agent skills and AI docs maintenance

- Keep `docs/ai/system_map.yaml` and `docs/ai/dev_commands.yaml` up to date when service structure, ports, or commands change.
- Keep `.codex/skills/ark-build/SKILL.md` and `.codex/skills/ark-debug/SKILL.md` aligned with current workflows.
